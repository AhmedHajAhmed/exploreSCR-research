...
def train_and_evaluate_table(df: pd.DataFrame) -> None:
    ...
    results = {classifier_name: {} for classifier_name in classifiers.keys()}
    ...
    for classifier_name, classifier in classifiers.items():
        for model_name, (X_train_vec, X_test_vec) in models.items():
            try:
                ...
                results[classifier_name][model_name] = accuracy
            except Exception as e:
                print(f"Error with {model_name} + {classifier_name}: {e}")
                results[classifier_name][model_name] = 'Error'

    # Creating and populating the PrettyTable
    table = PrettyTable()
    table.field_names = ["Classifier"] + list(models.keys())
    for classifier_name, accuracies in results.items():
        row = [classifier_name] + [accuracies.get(model_name, 'N/A') for model_name in models.keys()]
        table.add_row(row)
